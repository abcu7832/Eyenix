...
module MtoN_async_fifo #(
    ...
    localparam META_MSB = FIFO_MEM_SIZE - 1;
    localparam META_LSB = FIFO_MEM_SIZE - M_WRITERS;
    
    reg st_q, st_d;

    reg rinc, rinc_d;

    reg [FIFO_MEM_SIZE-1:0] packet_q, packet_d;
    reg [M_WRITERS-1:0]     consumed_q, consumed_d;

    wire [FIFO_MEM_SIZE-1:0] rd_data;
...
module OnetoN_async_fifo #(
    ...
    localparam FIFO_MEM_SIZE = WR_WIDTH;

    // =========================================================================
    // READ control (rd_clk) : 1:N
    // =========================================================================
    function automatic [$clog2(N_READERS)-1:0] rd_bit_cnt(input [N_READERS-1:0] g);
        integer i;
        begin
            rd_bit_cnt = 0;
    ...
    function automatic [$clog2(N_READERS)-1:0] lane_bit_cnt(input [LANES-1:0] g);
        integer i;
        begin
            lane_bit_cnt = 0;
            for (i=0;i<N_READERS;i=i+1) begin
    ...
    wire [FIFO_MEM_SIZE-1:0] rd_data;
    reg [FIFO_MEM_SIZE-1:0] packet_q, packet_d;    // 현재 packet data
    reg [FIFO_MEM_SIZE-1:0] packet1_q, packet1_d;  // 다음 packet data
    ...
    reg [$clog2(N_READERS)-1:0] req_cnt;
    reg [$clog2(LANES)-1:0] rem0;
    reg lane_found;
    ...
    always @(*) begin
        ...
        rem0     = (st_q==RD_HAVE) ? (LANES - rd_bit_cnt(consumed_q)) : 0;
